...
MODE="${1:-${FILEYARD_PRE_PUSH_MODE:-standard}}"
...
if [ "${FILEYARD_IN_CONTAINER:-0}" != "1" ] && [ "${FILEYARD_ALLOW_HOST_EXECUTION:-0}" = "1" ]; then
  echo "⚠️ pre_push_gate: emergency host execution enabled (FILEYARD_ALLOW_HOST_EXECUTION=1)." >&2
fi
...
RANGE_FLAGS=()
if [ "${FILEYARD_REQUIRE_NON_EMPTY_RANGE:-0}" = "1" ]; then
...
step() {
  ...
  case "$name" in
    ...
    feature-state-layout)
      echo "Hint: durable workbench state must stay under <workspace-root>/.fileyard, not repo root or repo runtime cache." >&2
      ;;
...
case "$MODE" in
  ...
  *)
    ...
    echo "" >&2
    echo "Set FILEYARD_PRE_PUSH_MODE env var to change default." >&2
    exit 2
    ;;
esac
