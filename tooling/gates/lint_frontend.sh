...
if [ "${FILEYARD_IN_CONTAINER:-0}" != "1" ]; then
  ...
  if [ "${LINT_FRONTEND_ALLOW_HOST_EXECUTION:-0}" = "1" ]; then
    ...
    env -u FILEYARD_ALLOW_HOST_EXECUTION FILEYARD_COMPOSE_SERVICE=fileyard-web-api bash "$ROOT/scripts/container_exec.sh" --label lint-frontend -- bash tooling/gates/lint_frontend.sh "$@"
    rc=$?
    cleanup_host_webui_mountpoint
    exit "$rc"
  fi
