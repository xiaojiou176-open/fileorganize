...
ensure_matrix_image() {
  local py_ver="$1"
  local image_var_name="FILEYARD_CI_IMAGE_PY${py_ver/./}"
  ...
  local_tag="fileyard-ci:local-py${py_ver/./}"
...
matrix_venv_volume() {
  local py_ver="$1"
  printf 'fileyard-matrix-venv-py%s' "${py_ver/./}"
}
...
run_one() {
  ...
  if docker run --rm \
    ...
    bash -lc '
      set -euo pipefail
      # Matrix contract note: export FILEYARD_VENV_DIR="/opt/fileyard-ci-venv"
      # The py3.10/py3.12 local matrix consumes the image-baked, hash-locked runtime
      # directly from /opt/fileyard-ci-venv. Do not shadow it with a mounted repo/runtime venv.
      ...
      /opt/fileyard-ci-venv/bin/pytest -q -o addopts= --maxfail=1 --strict-config --strict-markers tests/unit
