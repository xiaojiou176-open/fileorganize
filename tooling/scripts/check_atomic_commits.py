...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--max-files",
        type=int,
        default=_parse_env_int("FILEYARD_ATOMIC_MAX_FILES", 40),
        help="Per-commit max changed files (excluding allowlist).",
    )
    parser.add_argument(
        "--max-lines",
        type=int,
        default=_parse_env_int("FILEYARD_ATOMIC_MAX_LINES", 4500),
...
def main(argv: list[str]) -> int:
    ...
    allowlist.extend(_parse_allowlist([os.getenv("FILEYARD_ATOMIC_ALLOWLIST", "")]))
