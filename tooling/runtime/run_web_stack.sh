...
ACTION="up"
DETACH=0
API_HOST="${MOVI_WEB_API_HOST:-127.0.0.1}"
WEB_HOST="${MOVI_WEBUI_HOST:-127.0.0.1}"
API_PORT="${MOVI_WEB_API_PORT:-18080}"
WEB_PORT="${MOVI_WEBUI_PORT:-5173}"
...
run_compose() {
  ...
  if [ "$ACTION" = "down" ]; then
    echo "==> [web-stack-compose] phase=down services=movi-webui,movi-web-api"
    env COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME_FALLBACK" docker compose "${COMPOSE_ARGS[@]}" stop movi-webui movi-web-api >/dev/null 2>&1 || true
    env COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME_FALLBACK" docker compose "${COMPOSE_ARGS[@]}" rm -f movi-webui movi-web-api >/dev/null 2>&1 || true
    echo "✅ [web-stack-compose] action=down finished"
    return 0
  fi

  detect_static_mount_gap
  local up_args=(up --build movi-web-api movi-webui)
  if [ "$DETACH" = "1" ]; then
    up_args=(up --build -d movi-web-api movi-webui)
  fi
  ...
  MOVI_WEB_API_HOST=0.0.0.0 \
  MOVI_WEBUI_HOST=0.0.0.0 \
  MOVI_WEB_API_PORT="$API_PORT" \
  MOVI_WEBUI_PORT="$WEB_PORT" \
