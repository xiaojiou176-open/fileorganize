...
resolve_var_prefer_env_then_runtime_env MOVI_LIVE_TEST_URL ""
...
if [ -z "${MOVI_LIVE_TEST_URL}" ]; then
  echo "❌ run_live_tests: MOVI_LIVE_TEST_URL is required for live browser tests" >&2
  echo "Tip: set MOVI_LIVE_TEST_URL in $RUNTIME_ENV_FILE or current shell env." >&2
  exit 1
fi
...
if ! validate_url "${MOVI_LIVE_TEST_URL}"; then
  echo "❌ run_live_tests: MOVI_LIVE_TEST_URL must be an absolute https URL to a real external host." >&2
  echo "Current value: ${MOVI_LIVE_TEST_URL}" >&2
  exit 1
fi
...
export MOVI_RUN_LIVE_TESTS=1
export MOVI_RUN_WEBUI_E2E=1
LIVE_COVERAGE_FILE="${MOVI_LIVE_COVERAGE_FILE:-$(governance_runtime_ci_path "$REPO_ROOT")/.coverage-live-$$}"
...
mkdir -p "$LOG_DIR"
host="$("$VENV/bin/python" - "${MOVI_LIVE_TEST_URL}" <<'PY'
from urllib.parse import urlparse
import sys
