...
if [ "${FILEYARD_IN_CONTAINER:-0}" != "1" ]; then
  ...
  if [ "${FILEYARD_ALLOW_HOST_EXECUTION:-0}" = "1" ]; then
    if is_ci_context; then
      echo "❌ run_webui_task: FILEYARD_ALLOW_HOST_EXECUTION=1 is forbidden in CI" >&2
      exit 1
    fi
    ...
    if [ "$SKIP_INSTALL" = "1" ] && [ "$TASK" != "ci-install" ]; then
      ...
      env FILEYARD_COMPOSE_SERVICE=fileyard-web-api bash "$ROOT/scripts/container_exec.sh" --label "webui-${TASK}" -- bash -lc "${CONTAINER_INSTALL_COMMAND} && ${CONTAINER_SKIP_INSTALL_COMMAND}"
    else
