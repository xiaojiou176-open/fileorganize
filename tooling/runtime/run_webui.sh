...
HOST="${MOVI_WEBUI_HOST:-127.0.0.1}"
PORT="${MOVI_WEBUI_PORT:-5173}"
SKIP_INSTALL=0
...
if [ "${MOVI_IN_CONTAINER:-0}" != "1" ]; then
  ...
  env MOVI_COMPOSE_SERVICE=movi-webui bash "$ROOT/scripts/container_exec.sh" --label run-webui -- bash tooling/runtime/run_webui.sh "$@"
  run_rc=$?
