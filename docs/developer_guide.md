---
title: Movi Developer And Agent Guide
description: Choose the right Movi surface for operators, developers, and agents without breaking review-first safety.
---
...
| :-- | :-- | :-- |
| a Codex workflow | [Movi MCP For Codex](./codex_mcp.md) | it gives the client-specific MCP install path without pretending Codex can bypass review |
| a Claude Code workflow | [Movi MCP For Claude Code](./claude_code_mcp.md) | same local-first MCP lane, but written in Claude Code terms |
| a generic MCP-capable agent | [Movi MCP v1](./mcp.md) | this is the current stdio-first extension contract |
...
- **MCP facade**: `Movi MCP v1` reuses the same review-safe workflow rather than inventing a second business-logic path.
