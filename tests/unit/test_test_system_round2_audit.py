...
def test_quality_gate_runs_fast_lane_before_full_pytest() -> None:
    ...
    full_lane_pattern = (
        ...
        r"\s+\\\s*env -u PRE_COMMIT_FROM_REF -u PRE_COMMIT_TO_REF MOVI_RUN_LIVE_TESTS=0"
    )
...
def test_pre_commit_ai_context_and_ci_change_detection_are_config_driven() -> None:
    ...
    assert 'step fast-lane env MOVI_ALLOW_HOST_EXECUTION=1 bash "$ROOT/scripts/local_quality_gate.sh" fast' not in pre_push_gate
...
def test_prepush_gate_defaults_to_standard_mode() -> None:
    ...
    _assert_contains(pre_push_gate, 'MODE="${1:-${MOVI_PRE_PUSH_MODE:-standard}}"', "pre-push default mode standard")
...
def test_local_ci_matrix_gate_enforces_strict_pytest_markers_and_config() -> None:
    ...
    _assert_contains(
        matrix_gate,
        "directly from /opt/movi-ci-venv",
        "local matrix uses the image-baked runtime venv directly",
    )
    _assert_contains(
        matrix_gate,
        'export MOVI_VENV_DIR="/opt/movi-ci-venv"',
...
def test_live_integration_requires_secrets_and_uses_image_contract() -> None:
    ...
    _assert_contains(live_ci, "missing required secret MOVI_LIVE_TEST_URL", "live integration url preflight")
...
def test_env_contract_live_coverage_vars_and_budgets_are_consistent() -> None:
    ...
    assert "MOVI_LIVE_COVERAGE_FILE" in contract
    assert len(contract) <= 59

    live_count = sum(1 for name in contract if name.startswith("LIVE_"))
    movi_count = sum(1 for name in contract if name.startswith("MOVI_"))
    assert live_count <= budgets["LIVE_"]
    assert movi_count <= budgets["MOVI_"]
