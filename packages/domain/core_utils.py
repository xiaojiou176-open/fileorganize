        ...
        if metadata is not None:
            ...
            if stale:
                ...
                try:
                    ...
                    raise RuntimeError(f"Another Movi task is already running; lock file exists: {lock_path}") from race_exc
            else:
                raise RuntimeError(f"Another Movi task is already running; lock file exists: {lock_path}") from exc
        else:
            stale = False
            try:
                stat = lock_path.stat()
