...
def run_bundle_root() -> Path:
    raw = os.environ.get("MOVI_RUN_BUNDLE_ROOT", "").strip()
    if raw:
        return Path(raw).expanduser()
    return _workspace_root() / ".movi" / "runs"
...
def initialize_run_bundle(run_id: str, command: str, *, gate_run_id: str | None = None, gate_name: str | None = None) -> dict[str, str]:
    ...
    os.environ["MOVI_RUN_DIR"] = str(bundle_dir)
    os.environ["MOVI_RUN_EVENTS_PATH"] = str(events_path)
    os.environ["MOVI_RUN_STDERR_PATH"] = str(stderr_path)
    os.environ["MOVI_RUN_SUMMARY_PATH"] = str(summary_path)
    os.environ["MOVI_RUN_EVIDENCE_INDEX_PATH"] = str(evidence_index_path)
