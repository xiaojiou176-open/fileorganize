...
description: Teach an agent to install Fileyard's local MCP server, stay review-first, and use the safest manifest and batch-analysis tools before deeper mutation.
version: 1.0.0
triggers:
  - fileyard
  - fileyard organizer
...
# Fileyard Review-First Bundle

Teach the agent how to install, connect, and use Fileyard as a local-first
...
- how to wire Fileyard MCP into Codex, Claude Code, OpenHands, or OpenClaw
- which Fileyard tools are safe first when the work must stay review-first
...
Use Fileyard to inspect the current review-first workload. Start with `jobs.list`,
...
- the host can launch the local Fileyard MCP server from the provided config
...
- Fileyard stays a local-first review-first MCP workflow, not a hosted SaaS
