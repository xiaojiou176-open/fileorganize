...
ARTIFACT_ROOT = Path(os.environ.get("MOVI_ARTIFACT_ROOT", str(WORKSPACE_ROOT / ".movi" / "artifacts"))).expanduser()
...
def _refresh_runtime_defaults(*, input_root: Path | None = None, output_root: Path | None = None) -> None:
    ...
    if input_root is not None:
        ...
        os.environ["MOVI_INPUT_ROOT"] = str(DEFAULT_INPUT_ROOT)
    if output_root is not None:
        DEFAULT_OUTPUT_ROOT = output_root
        os.environ["MOVI_OUTPUT_ROOT"] = str(DEFAULT_OUTPUT_ROOT)
...
def _bootstrap_runtime_defaults() -> None:
    ...
    input_root = _normalize_runtime_path(runtime_env_values.get("MOVI_INPUT_ROOT"), DEFAULT_INPUT_ROOT)
    output_root = _normalize_runtime_path(runtime_env_values.get("MOVI_OUTPUT_ROOT"), DEFAULT_OUTPUT_ROOT)
...
def _runtime_settings_view() -> RuntimeSettingsView:
    ...
    input_root_raw, _ = _resolve_runtime_value("MOVI_INPUT_ROOT", str(DEFAULT_INPUT_ROOT))
    output_root_raw, _ = _resolve_runtime_value("MOVI_OUTPUT_ROOT", str(DEFAULT_OUTPUT_ROOT))
...
def _update_runtime_settings(payload: RuntimeSettingsUpdateRequest) -> RuntimeSettingsView:
    ...
    runtime_env_values["MOVI_INPUT_ROOT"] = str(next_input_root)
    runtime_env_values["MOVI_OUTPUT_ROOT"] = str(next_output_root)
...
def create_app(command_executor: CommandExecutor | None = None) -> FastAPI:
    app = FastAPI(title="Movi Web API", version="2.0.0")
    router = APIRouter()
    ...
    def healthz() -> Dict[str, str]:
        return {"status": "ok", "service": "movi-web-api"}
    ...
    def create_rollback_job(payload: RollbackRequest) -> JobView:
        ...
        if strict_integrity and not _has_strong_rollback_signing_key():
            raise HTTPException(status_code=400, detail="strict_integrity=true requires MOVI_ROLLBACK_HMAC_KEY")
        job_payload = {
            "manifest_path": str(manifest_path),
            "execute": execute,
    ...
    def list_learned_rules() -> Dict[str, Any]:
        ...
        return {
            "items": rules,
            "count": len(rules),
            "path": str(WORKSPACE_ROOT / ".movi" / "preferences" / "learned_rules.json"),
        }
    ...
    def list_watch_sources_route() -> Dict[str, Any]:
        ...
        return {"items": items, "count": len(items), "path": str(WORKSPACE_ROOT / ".movi" / "preferences" / "watch_sources.json")}
