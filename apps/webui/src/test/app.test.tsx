...
describe('App routes', () => {
  ...
  it('renders dashboard hero text and syncs route metadata', async () => {
    ...
    expect(document.title).toBe('Fileyard | Review-first local organizer')
    expect(document.querySelector('meta[name="description"]')?.getAttribute('content')).toContain('review-first local file organizer')
    expect(document.querySelector('meta[property="og:title"]')?.getAttribute('content')).toBe('Fileyard | Review-first local organizer')
