...
describe('ReviewQueuePage', () => {
  ...
  it('renders copilot guidance, learned explainability, and bucket sections', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'Movi Review' })).toBeInTheDocument()
    expect(screen.getByRole('heading', { name: 'Movi Copilot v1' })).toBeInTheDocument()
  ...
  it('renders localized review workbench copy when locale switches to zh-CN', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'Movi 审核台' })).toBeInTheDocument()
    await waitFor(() => {
      expect(screen.queryByRole('heading', { name: '正在加载 Review Queue' })).not.toBeInTheDocument()
    })
    expect(await screen.findByRole('heading', { name: 'Movi Copilot v1' }, { timeout: 5000 })).toBeInTheDocument()
