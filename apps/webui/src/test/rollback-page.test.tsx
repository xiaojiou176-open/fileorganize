...
const mocks = vi.hoisted(() => ({
  ...
  getRuntimeSettingsMock: vi.fn(async () => ({
    ...
    runtime_env_path: '/tmp/workspace/.movi/env/runtime.env',
    ...
    manifest_root: '/tmp/workspace/.movi/manifests',
    artifact_root: '/tmp/workspace/.movi/artifacts',
    has_api_key: true,
    api_key_masked: '***',
  ...
  it('surfaces rollback context while keeping real rollback locked by default', async () => {
    mocks.getJobMock.mockResolvedValueOnce(
      makeJob({
        id: 'source-job',
        status: 'succeeded',
        manifest_path: '/tmp/workspace/.movi/manifests/manifest.jsonl',
        rollback_manifest_path: '/tmp/workspace/.movi/manifests/rollback.jsonl',
        summary: {
          total: 2,
          with_error: 0,
          by_media_type: {},
