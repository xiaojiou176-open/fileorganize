...
def _cleanup_repo_local_pycache() -> None:
    repo_root = _repo_root()
    for rel in ("__pycache__", "tooling", "packages", "apps", "tests"):
        ...
        if not target.exists():
            continue
        for found in target.rglob("__pycache__"):
...
atexit.register(_cleanup_repo_local_pycache)
